...
const servicebusCommon = require('./servicebus_common')
...
class QueueServiceBus {
  ...
  */
  ...
  listQueues(options, cb) {
    this._sbs.listQueues(options, (err, result, response) => {
      cb(sanitizeServiceBusError(err), result, response)
    })
  }

  listQueuesAndMessageCount(options, cb) {
    this.listQueues(options, (err, res) => {
      ...
      res = _.map(res, info => {
        return {
          name: info.QueueName,
          messages: {
            active: info.CountDetails['d2p1:ActiveMessageCount'],
            deadLetter: info.CountDetails['d2p1:DeadLetterMessageCount']
          }
        }
      })
      cb(err, res)
  ...
  lockMessage(queueName, cb) {
    ...
    this.lockBrokeredMessage(queueName, (err, fullMsg, msgHandler) => {
      ...
      if(!err) {
        ...
        cb(err, msg, msgHandler, fullMsg.brokerProperties.MessageId)
      } else {
        cb(sanitizeServiceBusError(err))
      }
    });
